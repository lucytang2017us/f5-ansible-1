...
for in_file in args.i:
    ...
    try:
        for row in reader:
            row["http_profile"] = append_shared_path(row["shared_path"], row["http_profile"])
            row["tcp_profile"] = append_shared_path(row["shared_path"], row["tcp_profile"])
            row["persist_profile"] = append_shared_path(row["shared_path"], row["persist_profile"])
            row["policy_vs"] = append_shared_path(row["shared_path"], row["policy_vs"])
            row["cert_chain"] = append_shared_path(row["shared_path"], row["cert_chain"])
            row["cert_name"] = append_shared_path(row["shared_path"], row["app_cert"]) + ".crt"
            row["cert_key"] = append_shared_path(row["shared_path"], row["app_cert"]) + ".key"
